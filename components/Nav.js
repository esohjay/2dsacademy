...
const Nav = ({ background }) => {
  ...
  return (
    ...
    <header className={`h-[60px] ${fixedNav} relative z-40  `}>
      ...
      >
        {/* Logo */}
        <h3 className="font-bold text-xl">2dsAcademy</h3>
        ...
        {/* Hamburger Icon */}
        <Link href="#enroll">
          <a className="py-2 text-center  bg-altColor px-7 text-white font-bold rounded-md hidden md:block">
            Enroll
          </a>
        </Link>
      ...
      <nav
        className={`${
          showNav ? "flex" : "hidden"
        } w-[100%] transition-all duration-1000 `}
      >
        <ul className="flex flex-col w-[100%] justify-center items-center bg-white py-6 ">
          <li
            onClick={() => setShowNav(false)}
            className=" w-[100%] px-5 text-mainColor hover:bg-mainColor hover:text-white cursor-pointer font-medium transition-all duration-500 "
          >
            <Link href="/">
          ...
          <li
            onClick={() => setShowNav(false)}
            className=" w-[100%] px-5 text-white bg-altColor hover:bg-mainColor  cursor-pointer font-medium transition-all duration-500 "
          >
            <Link href="#enroll">
              <a className="py-5 block text-center   px-7 ">Enroll</a>
            </Link>
          </li>
        </ul>
      </nav>
